...
export default function FacilitiesSlider() {
  ...
  // Calculate the transform offset to center the highlighted facilities
  const calculateCenterOffset = (setIndex: number) => {
    const relatedFacilities = facilityImageSets[setIndex].relatedFacilities;
    const middleFacilityIndex = relatedFacilities[1]; // Middle facility of the 3
    const facilityWidth = 152; // min-w-[140px] + mx-6 (24px) = 152px per facility
    const containerWidth = typeof window !== 'undefined' ? window.innerWidth : 1200;
    const centerPosition = containerWidth / 2;
    
    // Calculate offset to center the middle facility
    const offsetToCenter = centerPosition - (middleFacilityIndex * facilityWidth) - (facilityWidth / 2);
  ...
  return (
    <div className="w-full bg-white py-16">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Synchronized Facilities Carousel */}
        <div className="relative w-full mb-16 overflow-hidden py-10">
          ...
          {/* Centered Facilities Track */}
          <div className="flex justify-center">
            <div 
              className="flex transition-transform duration-700 ease-in-out"
              style={{
                transform: `translateX(${calculateCenterOffset(currentSetIndex)}px)`
              }}
            >
              {facilities.map((facility, index) => {
                const isHighlighted = highlightedFacilities.includes(index);
                  ...
                  <div
                    ...
                    className={`flex flex-col items-center justify-center min-w-[140px] mx-6 transition-all duration-500 ${
                      isHighlighted
                        ...
                        : "text-gray-700 scale-100 opacity-50"
                    }`}
                  >
                    <div
